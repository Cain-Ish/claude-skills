---
name: task-analyzer
description: Analyzes request complexity and recommends optimal agent coordination pattern with cost-benefit analysis
color: blue
tools:
  - Bash
  - Read
...
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/lib/complexity-analyzer.js "<user_request>" <token_budget>
```
...
**Request**: "Add a console.log statement to debug the authentication flow"
...
```json
{
  ...
  "within_budget": true,
  "reasoning": "Simple debugging task requiring single code addition. No multi-agent coordination needed.",
  "warnings": [],
  "alternatives": []
}
```
...
**Request**: "Implement user registration endpoint and add unit tests"
...
```json
{
  ...
  "pattern": "sequential",
  "recommended_agents": ["general-purpose", "test-automator"],
  "cost": {
    "single": 13000,
    "multi": 50000,
    "multiplier": "4x"
  ...
  "within_budget": true,
  "reasoning": "Two-step workflow: implement endpoint first, then create comprehensive tests. Sequential pattern ensures tests are written against completed code.",
  "warnings": ["4x token cost - ensure tests justify the investment"],
  "alternatives": [
    {
      "pattern": "single",
      "agents": ["general-purpose"],
      "cost": 13000,
      "trade_offs": "Single agent can handle both, but dedicated test-automator provides better coverage"
    }
  ]
}
```
...
**Request**: "Comprehensive code review of authentication module including security audit, performance analysis, and test coverage validation"
...
```json
{
  ...
  "domains": ["security", "performance", "testing", "review"],
  "pattern": "parallel",
  "recommended_agents": ["security-auditor", "performance-engineer", "test-automator"],
  "cost": {
    "single": 20000,
    "multi": 180000,
    "multiplier": "9x"
  ...
  "within_budget": true,
  "reasoning": "Four independent analysis domains (security, performance, testing, code quality). Parallel execution provides comprehensive coverage with each specialist focusing on their domain. 90% quality improvement justifies 9x token cost.",
  "warnings": ["High token usage - verify this level of analysis is needed"],
  "alternatives": [
    {
      "pattern": "sequential",
      "agents": ["security-auditor", "code-reviewer"],
      "cost": 90000,
      "trade_offs": "Focus on security first, then general review. Faster and cheaper but less comprehensive."
    }
  ]
}
```
